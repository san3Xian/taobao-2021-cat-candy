...
img_taobao_guang_guang_text = "taobao-pics/guangguang.png"
...
while(1):
    ...
    # 针对 逛逛 类型任务进行识别(更换手机可能需要调整阈值)
    click_location_guang = cv_img_match(
        img_source, img_taobao_guang_guang_text, methods[2], ocv_image_read_type[1], thresold=0.784)
    print("Task start button location(guang_guang_text): ", click_location_guang)

    # 针对 浏览15秒(s) 类型任务进行识别(更换手机可能需要调整阈值)
    ...
    click_location = click_location_15s + click_location_go_view + click_location_guang
    ...
    for task_loc in click_location:
        ...
        this_task_state = taobao_get_task_state(
            task_loc, task_done_locations, done_task_icon_height)
